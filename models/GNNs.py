...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=3, heads=1, dropout=0.5):
        ...
        self.convs = nn.ModuleList()
        if n_layers == 1:
            # Single GAT layer for input to output directly
            self.convs.append(GATConv(in_channels, out_channels, heads=heads, dropout=dropout, concat=False))
        else:
            # First GAT layer (input to hidden)
            self.convs.append(GATConv(in_channels, hidden_channels, heads=heads, dropout=dropout))
            # Intermediate GAT layers (hidden to hidden)
            for _ in range(n_layers - 2):
                self.convs.append(GATConv(hidden_channels * heads, hidden_channels, heads=heads, dropout=dropout))
            # Last GAT layer (hidden to output)
            self.convs.append(GATConv(hidden_channels * heads, out_channels, heads=heads, dropout=dropout, concat=False))
...
class GraphSAGE(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=3, dropout=0.5):
        ...
        self.convs = nn.ModuleList()
        if n_layers == 1:
            # Single SAGE layer for input to output directly
            self.convs.append(SAGEConv(in_channels, out_channels))
        else:
            # First SAGE layer (input to hidden)
            self.convs.append(SAGEConv(in_channels, hidden_channels))
            # Intermediate SAGE layers (hidden to hidden)
            for _ in range(n_layers - 2):
                self.convs.append(SAGEConv(hidden_channels, hidden_channels))
            # Last SAGE layer (hidden to output)
